# -*- coding: utf-8 -*-
"""
Created on Thu Apr  8 01:34:59 2021
...
class MotifFinding:
    ...
    def score_PseudoCounts(self, s):
        score = 0
        motif = self.createMotifFromIndexes(s)
        motif.doPseudoCounts()
        mat = motif.counts
    ...
    def heuristicStochastic_PseudoCounts(self):
        ...
        best_score = self.score_PseudoCounts(s) # calcula o score com base nas posiçoes s 
    ...
    def gibbs_PseudoCounts(self):
        ...
        for i in range(len(self.seqs)):
            s[i] = randint(0, self.seqSize(i) - self.motifSize) # de 0 ate ao tamanho maximo (no randint o maximo ja e incluisve nas lista nao incluem o ultimo numero por isso coloca-se mais um)
        best_score = self.score_PseudoCounts(s) 
        improve = True 
        while improve:
            # passo 2: 
...
def test5():
    mf = MotifFinding()
    mf.readFile("exemploMotifs.txt", "dna")
    print(" Heuristic stochastic - PseudoCounts")
    sol = mf.heuristicStochastic_PseudoCounts()
    print("Solution: ", sol)
    print("Score:", mf.score_PseudoCounts(sol))
    print("Score mult:", mf.scoreMult(sol))
    print("Consensus:", mf.createMotifFromIndexes(sol).consensus())

    print("Gibbs - PseudoCounts")
    sol2 = mf.gibbs_PseudoCounts()
    print("Score:", mf.score_PseudoCounts(sol2))
    print("Score mult:", mf.scoreMult(sol2))
    print("Score mult:", mf.scoreMult(sol2))
    print("Consensus:", mf.createMotifFromIndexes(sol2).consensus())
