# -*- coding: utf-8 -*-
"""
Created on Thu Apr 29 13:57:50 2021
...
class MyGraph:
    ...
    def add_edge(self, o, d):
        ...
        if o not in self.graph.keys(): # verifica se existe o vertice o
            self.add_vertex(o)         # se nao adiciona o vertice ao dicionario 
        if d not in self.graph.keys(): # verifica se existe o vertice d
            self.add_vertex(d)         # se nao adiciona o vertice ao dicionario 
        if d not in self.graph[o]:     # se o grafo nao existir   
            self.graph[o].append(d)    # adicona o grafo  
    ...
    def distance(self, s, d): # s é o no inicial 
        if s == d: return 0
        l = [(s,0)]
        visited = [s]
        while len(l) > 0:
            ...
            for elem in self.graph[node]:
                if elem == d: 
                    ...
                    l.append((elem,dist+1)) # adiona ao caminho o nó que estamos a ver 
                    visited.append(elem)
        # se o while chegar ao fim nao se atingiu o no             
        return None
    ...
    def shortest_path(self, s, d):
        ...
        visited = [s]
        while len(l) > 0:
            node, preds = l.pop(0) # preds caminho dos nos 
            for elem in self.graph[node]:
                if elem == d: 
                    return preds+[node,elem]
                elif elem not in visited:
                    l.append((elem,preds+[node])) # adiciona o elemento e o no ao caminho 
                    visited.append(elem)
        return None
