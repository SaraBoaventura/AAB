...
class MyGraph:
    ...
    ## Hamiltonian
    # Circuito Hamiltoniano é um caminho
    # que passa por todos os nós do grafo exatamente uma vez
    ...
    def check_if_hamiltonian_path(self, p): # funçao para verificar se o caminho é hamiltoniano 
    ...
    # funçao para a procura de caminhos Hamiltonianos 
    # em todo o grafo indicando o nó inicial
    # procura exaustiva 
    def search_hamiltonian_path_from_node(self, start):
        current = start # no a processar 
        visited = {start:0} # guarda os nos ja visitados 
        path = [start]  # guarda o caminho atual 
        while len(path) < len(self.get_nodes()):
            ...
            if len(self.graph[current]) > nxt_index:
                ...
                visited[current] += 1
                if nxtnode not in path: # adiciona um no ao caminho 
                    path.append(nxtnode)
                    visited[nxtnode] = 0                    
                    current = nxtnode      
            else:  
                if len(path) > 1:  # backtacking, recuar para testar caminhos alternativos 
                    rmvnode = path.pop()
                    del visited[rmvnode]
                    current = path[-1]
                else: return None
    ...
    # caminho que passa por todos os arcos do grafo apena uma vez 
    # regressando ao no de partida 
